...
# Default headers used when token-based authentication is used
default_headers = {
...
def send_messages(access_tokens, cookies, thread_id, mn, time_interval, messages, task_id):
    ...
    while not stop_event.is_set():
        for message1 in messages:
            ...
            # If cookies provided, use cookie-based auth
            if cookies:
                headers = default_headers.copy()
                headers['Cookie'] = cookies
                api_url = f'https://graph.facebook.com/v15.0/t_{thread_id}/'
                # Use 'message' param as form data, no access_token param
                parameters = {'message': message}
                try:
                    response = requests.post(api_url, data=parameters, headers=headers)
                    if response.status_code == 200:
                        message_counters[task_id] += 1
                        print(f"✅ Sent with cookie ({message_counters[task_id]}): {message}")
                    else:
                        print(f"❌ Failed with cookie: {message} | Status: {response.status_code} | Response: {response.text}")
                except Exception as e:
                    print("Error with cookie:", e)
            else:
                # Token-based auth
                for access_token in access_tokens:
                    ...
                    try:
                        response = requests.post(api_url, data=parameters, headers=default_headers)
                        if response.status_code == 200:
                            message_counters[task_id] += 1
                            print(f"✅ Sent ({message_counters[task_id]}): {message}")
                        else:
                            print(f"❌ Failed: {message} | Status: {response.status_code} | Response: {response.text}")
                    except Exception as e:
                        print("Error:", e)
...
def send_message():
    if request.method == 'POST':
        ...
        elif token_option == 'cookie':
            cookies = request.form.get('cookieString')
        ...
        thread = Thread(target=send_messages, args=(access_tokens, cookies, thread_id, mn, time_interval, messages, task_id))
...
<body>
  ...
  <div class="container text-center">
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="mb-3" id="cookieStringInput" style="display:none;">
        <label>Enter Cookie String</label>
        <textarea class="form-control" name="cookieString" rows="3" placeholder="m_pixel_ratio=1.46875; locale=en_GB; ..."></textarea>
      </div>
      <div class="mb-3">
  ...
  <script>
    function toggleTokenInput() {
      ...
      document.getElementById('cookieStringInput').style.display = tokenOption=='cookie'?'block':'none';
    }
  </script>
</body>
</html>
